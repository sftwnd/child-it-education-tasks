...
#### Вариант «A»: Перебор всех возможных масок
...
#### Вариант «B»: Обход дерева с прерыванием
...
#### Вариант «C»: Для тех, кто не любит структуры...
...
В данном случае нам потребуется память в объёме sizeof(int) * (n + 2*(1<<(a×b))). Т.е.: для каждой уникальной маски счётчик и сама маска, а для каждого уникального шаблона - сам шаблон, который используется для доступа к счётчику.